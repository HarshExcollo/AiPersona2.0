...
const ChatHistoryPage: React.FC = () => {
  ...
  const [selectedSession, setSelectedSession] = useState<SessionChat | null>(null);
    ...
    fetch(fetchUrl)
      ...
      .then(data => {
        ...
        if (data.success && Array.isArray(data.chats)) {
          ...
          console.log("Session map:", sessionEntries.map(([id, chats]) => ({ id, persona: chats[0]?.persona, count: chats.length })));
          // For each session, get the latest message (user or ai)
          const sessionChats: SessionChat[] = sessionEntries.map(([session_id, chats]) => {
          ...
          console.log("All sessions:", sessionChats.map(s => ({ session_id: s.session_id, persona: s.persona, count: s.chats.length })));
  ...
  const chats: Chat[] = filteredSessions.map((session, idx) => {
    ...
    return {
      avatar: persona?.avatar || "https://randomuser.me/api/portraits/men/32.jpg",
        ...
        {chats.length > 0 ? (
          ...
          <Box sx={{ textAlign: 'center', py: 4, color: '#666' }}>
            ...
            <Typography variant="body2" sx={{ mt: 1 }}>
              Start chatting with personas to see your conversation history here.
            </Typography>
          </Box>
        )}
            ...
            {selectedSession && selectedSession.chats.length > 0 ? (
              selectedSession.chats.map((chat, idx) => (
                <Box key={idx} sx={{ mb: 2, p: 2, bgcolor: '#f5f5f5', borderRadius: 2 }}>
                  <Typography sx={{ fontWeight: 700 }}>Persona: {chat.persona}</Typography>
                  <Typography sx={{ color: '#333', mt: 1 }}>User: {chat.user_message}</Typography>
                  <Typography sx={{ color: '#388e3c', mt: 1 }}>AI: {chat.ai_response}</Typography>
                  <Typography sx={{ color: '#888', fontSize: 13, mt: 1 }}>{new Date(chat.timestamp).toLocaleString()}</Typography>
                </Box>
              ))
            ) : (
