...
export default function ChatPage({ onBack }: ChatPageProps) {
  ...
  const loadChatHistory = async (sessionId: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/personas/chats?user=${userId}&persona=${persona.id}&session_id=${sessionId}`);
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    const sessionId = currentSessionId || (messages.length === 0 ? startNewSession(persona.id) : getSessionId(persona.id));
    ...
    if (isWebhookPersona(personaId)) {
      try {
        ...
        const webhookResponse = await sendToWebhook(trimmed, personaId, personaName);
        ...
        const storePromise = fetch('http://localhost:3000/api/personas/chats', {
          ...
          if (!res.ok) {
            return res.text().then(text => { throw new Error('Failed to store chat: ' + text); });
          }
          return res.json();
        }).catch(err => {
